...
Pick a classic strategy game and there's a good chance an RL environment exists for it: Chess, Go, Shogi, even obscure things like Hnefatafl and Arimaa have been formalised in Gym/Gymnasium. Poker, Diplomacy, Catan, Hanabi — all well-trodden. But **Carrom**, a game played daily by hundreds of millions across India, Pakistan, Bangladesh, Sri Lanka, Nepal, and the diaspora, has been sitting in a blind spot.
...
- 🟢 **Green Agent** — the evaluator: defines tasks, environment, and scoring
- 🟣 **Purple Agent** — the competitor: the AI agent under test
- 🔴 **Red Agent** — adversarial tester (not used here)

`GreenCarromAgent` is my green agent. It owns:

1. **A task suite** — seeded boards in easy / standard / hard tiers
2. **The environment** — wraps `CarromEnv` with full ICF rules
3. **Scoring** — ICF-aware metrics (reward, win rate, ICF compliance from dues/fouls) plus compute efficiency (sim steps, wall time)
...
- **Numeric**: `positions`, `velocities`, `pocketed` flags — for standard RL agents
- **Text**: a human-readable `text_summary` with every coin's position, nearest pocket, and a plain-English rules reminder — for LLM agents
...
Actions come in two flavours: **numeric** (`placement_x`, `angle` in radians, `force` in `[0, 1]`), or **text** (`"aim at queen_0 with strong force from centre"` — the env parses it). Text actions bridge the LLM-to-physics gap: the model reasons in natural language, the environment grounds it in Pymunk.
...
**Why GRPO?** It needs no value network (no critic) — it samples G completions per prompt, scores each with the reward function, and uses the group's mean/std as the baseline. Simpler than PPO and works well for structured-output tasks like "return JSON with `placement_x`, `angle`, `force`".
...
**Cost estimates on Modal** (A10G @ ~$1.10/hr, Gemma-3-4B with Unsloth 4-bit):
...
The script supports Gemma-3-1B/4B, Qwen2.5-1.5B/3B/7B out of the box, with the model / step count / hardware tier all controllable from the command line.
...
**MiniMax plays carefully and wins.** 8 own-coins potted, one due, five fouls over 103 agent shots, 97% ICF compliance. The model clearly understands the rules — "don't pot the opponent's coins" — and it plays the scoring game over the reward-hacking game. Higher coins per simulation step than either baseline.
...
Both baselines won (reward > 0 and `agent_score > opponent_score`). That's a nice property of the evaluator: the baselines establish a meaningful floor, and capability gains between frontier models show up on the same scorecard.
...
- **Multi-agent self-play** — replace the scripted opponent with an older copy of the agent
- **Curriculum learning** — start with pre-set pocket-ready boards, grade up to full clears
...
- **torchforge integration** — distributed rollout collection for bigger models
...
*Questions or PRs welcome on [GitHub](https://github.com/bp-high/carrom_rl_env-). If you try your own model on this benchmark, I'd love to see the results.*
